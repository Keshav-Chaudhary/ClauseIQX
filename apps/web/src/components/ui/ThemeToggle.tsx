...
/**
 * Premium animated pill theme toggle with SSR hydration protection.
...
export function ThemeToggle({ className }: { className?: string }) {
  ...
  if (!mounted) {
    return (
      <div
        className={cn(
          "relative flex h-8 w-16 items-center rounded-full border border-[var(--border-strong)] bg-surface-3 p-0.5 opacity-80",
          className,
        )}
      >
        <div className="relative z-10 flex w-1/2 justify-center text-fg-muted">
          <Moon className="size-3.5" />
        </div>
        <div className="relative z-10 flex w-1/2 justify-center text-fg-subtle">
